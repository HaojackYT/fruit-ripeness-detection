...
try:
    ...
    from .feature_extraction import FruitFeatureExtractor
except ImportError:
    from color_space import rgb_to_hsv
    from geometric import align_orientation_pca
    from illumination import gamma_correction
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run preprocessing & feature extraction for Fruit Ripeness Detection.")
    ...
    parser.add_argument(
        "--save-hsv-npy",
        default=None,
        help="Optional .npy file to save final HSV float output.",
    )
    
    parser.add_argument(
        "--save-features",
        default=None,
        help="Optional .npy file to save the extracted feature vector for SVM.",
    )
    return parser
...
def main() -> int:
    ...
    feature_vector = extractor.get_all_features(hsv_final)
    ...
    if args.save_features:
        np.save(args.save_features, feature_vector)
