...
def _prepare_rgb_float(img: np.ndarray) -> np.ndarray:
    """
    Chuẩn hóa ma trận ảnh đầu vào (RGB) có kiểu dữ liệu float32 và các pixel nằm trong khoảng [0.0, 1.0].

    Tham số:
        img (np.ndarray): Ma trận ảnh đầu vào.

    Trả về:
        np.ndarray: Ma trận ảnh với kiểu dữ liệu float32 và các pixel nằm trong khoảng [0.0, 1.0].
